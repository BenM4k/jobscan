    ...
    >
      ...
      {displaySummary && (
        <section aria-labelledby="summary-heading" className="py-8 sm:py-10 space-y-4">
          <h2
            id="summary-heading"
            className="text-xs font-semibold text-slate-700 dark:text-zinc-300 flex items-center gap-2 font-sans"
          >
                ...
                <div key={idx} className="relative pl-6 space-y-2">
                  ...
                  <span
                    ...
                    className={`absolute left-0 top-0.5 text-[11px] select-none ${
                      isFirst ? "text-blue-600 dark:text-blue-400 font-black" : "text-slate-300 dark:text-zinc-600 font-bold"
                    }`}
                  >
                    ◆
                  </span>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-baseline gap-1">
                    ...
                    {dateRange && (
                      <span className="text-xs sm:text-[13px] font-medium text-slate-700 dark:text-zinc-300 font-sans">
                        {dateRange}
                      </span>
                    )}
                  </div>
            ...
            {education.map((edu, idx) => {
              ...
              return (
                <div key={idx} className="space-y-1.5">
                  ...
                  {dateRange && (
                    <p className="text-xs sm:text-[13px] font-medium text-slate-700 dark:text-zinc-300 font-sans">
                      {dateRange}
                    </p>
                  )}
                </div>
      ...
      <section aria-labelledby="skills-heading" className="pt-8 sm:pt-10 space-y-4">
        <h2
          ...
          className="text-xs font-semibold text-slate-700 dark:text-zinc-300 flex items-center gap-2 font-sans"
        >
          <Zap className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400 shrink-0 fill-blue-600/20" />
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {skills.length > 0 ? (
            skills.map((skill, idx) => (
              <span
                key={idx}
                className="text-xs bg-slate-100 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700/60 text-slate-800 dark:text-zinc-200 px-2.5 py-1 rounded-md font-medium shadow-2xs"
              >
                {skill}
              </span>
            ))
