...
- **Tailored Resume Edit Persistence & Client Sanitization:** User modifications to AI-tailored resumes persist via `PUT /api/jobs/[id]/tailor-resume` calling `jobsDal.updateJobTailoredResume` before exiting edit mode. External job descriptions rendered via `dangerouslySetInnerHTML` in client components must be sanitized with an explicit safe-tag/safe-attribute allowlist via browser `DOMParser`.
